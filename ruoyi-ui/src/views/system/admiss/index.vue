<template>
  <div class="app-container">
    ...
    <goods-select v-if="selectGoodsDialog" ref="selectGoods" @selectDataMore="selectDataMore" @selectData="selectData"></goods-select>
  </div>
</template>
  ...
  methods: {
    ...
    goodsSelect() {
      ...
      this.$nextTick(() => {
        ...
        if (this.userName != "") {
          this.$refs.selectGoods.queryParams.createBy = this.userName;
          this.$refs.selectGoods.queryParams.isSend = 1;
          this.$refs.selectGoods.getList();
        } else {
          //-1 代表无数据
          // this.$refs.selectGoods.queryParams.createBy = '-1';
        }
      });
    },
    //选择数据
